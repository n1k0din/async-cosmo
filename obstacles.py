...
class Obstacle:
    ...
    def get_bounding_box_frame(self) -> str:
        ...
        Increments box size to compensate obstacle movement.
        """
...
def _get_bounding_box_lines(rows: int, columns: int) -> Generator:
    yield ' ' + '-' * columns + ' '
    for _ in range(rows):
        yield '|' + ' ' * columns + '|'
    yield ' ' + '-' * columns + ' '
